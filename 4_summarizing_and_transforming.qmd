...
### `mutate()` is from `dplyr`* {style="margin-bottom:75px"}

### `mutate()` has a `.by` argument {.spacer}

[* part of the `tidyverse`]{.footnote}

```{r}
#| eval: false
mutate(data, column1 = expression1, column2 = expression2, ..., .by = c(group1, group2))
```
...
```{r, echo = FALSE, R.options = list(tibble.print_min = 9)}
...
[Always remember to **`ungroup()`** your data!]{.fragment .small .note .absolute left=85% top=35% width=20%}
...
[**Too Easy?**<br>Try both methods<br>Try grouping by multiple columns]{.note .absolute bottom=10% width=60%}
...
size <- read_csv("data/grain_size2.csv") |>
  ...
  mutate(total_sand = coarse_sand + medium_sand + fine_sand,
         total_silt = coarse_silt + medium_silt + fine_silt) |>
  mutate(mean_silt = mean(total_silt), .by = c(habitat, technician))
```

```{r}
#| echo: false
select(size, habitat, technician, contains("silt"))
```
...
#| code-line-numbers: "2"
size <- size |>
  summarize(mean_sand = mean(total_sand), .by = plot)
...
- Keep other id columns by adding them to `.by`
- Beware: think carefully about grouping variables!


```{r}
size |> 
  summarize(mean_sand = mean(total_sand), .by = c(plot, depth))
```
...
```{r}
size |> 
  summarize(mean_sand = mean(total_sand), .by = c(plot, habitat))
```
...
#| eval: false
size |>
  summarize(samples_total = n(), .by = plot)
```
:::
:::{.column}
```{r}
...
[**Hint**<br>[Use `sd(x)`/`sqrt(n())` for standard error calculations]{.small}]{.note .absolute left=85% bottom=60% width=40%}
...
- Is your data grouped when it shouldn't be? (try using `ungroup()`)
:::
