...
- [`rivers_correct.xlsx`](`r glue("{site}/data/rivers_correct.xlsx")`) {{< fa arrow-right >}} Excel file {{< fa arrow-right >}} `read_excel()`
- [`rivers_raw.csv`](`r glue("{site}/data/rivers_raw.csv")`) {{< fa arrow-right >}} Comma-separated-variables {{< fa arrow-right >}} `read_csv()`
...
[**OR**<br>Try to load the second sheet of `rivers_correct.xlsx` and rename the columns in the same step<br>[(see `?read_excel`)]{.small}]{.note .absolute bottom=-15% left=70% width=60%}
...
- skip the first *three* rows (including the headers)
- provide new header names

```{r}
excel_sheets("data/rivers_correct.xlsx")  # Or look yourself ;)

read_excel("data/rivers_correct.xlsx", sheet = "Oswegatchie", skip = 3, 
           col_names = c("name", "site", "element", "amount", 
                         "temp", "year", "weather"))
```
...
[[`rivers_raw.csv`](`r glue("{site}/data/rivers_raw.csv")`)]{.large}


## Look at the data

```{r, message = FALSE}
rivers <- read_csv("data/rivers_raw.csv")
rivers
```
...
[Note how code repeats data frame `rivers`...]{.note .fragment .absolute bottom=0}
...
rivers <- rename(rivers, element = ele, amount = amo, temperature = temperature_c)
rivers <- select(rivers, -wea)
rivers <- mutate(rivers, river_name = if_else(river_name %in% c("Grase", "grasse"), "Grasse",  river_name))
```
...
```{r}
filter(rivers, amount < 0.05 | is.na(amount))
```
:::
...
[Year could be categorical (factor)<br>Better for plotting!<br>[(although it really depends)]{.small}]{.note .absolute bottom=5%}
