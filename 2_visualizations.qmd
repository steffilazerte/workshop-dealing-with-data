...
![](figures/hex_ggplot2.png){width=20% .absolute top=0 right=0}
...
## {data-menu-title="ggplot2" background-image="figures/horst_ggplot2.png" background-size=75% background-position="top"}
...
![](figures/horst_lter_penguins.png){width=80%}
![](figures/hex_palmerpenguins.png){.absolute top=0 right=0 width=20%}
...
![](figures/hex_palmerpenguins.png){.absolute top="0" right="0" width="12%"}
...
[**Too Easy?**<br>Plot points on top]{.note .absolute left=80% top=5%}
...
[**Note**: When forcing, aesthetic is not inside `aes()`]{.note .absolute left=70% bottom=20%}
...
[**Too Easy?**<br>Create a plot of your own data
]{.note .absolute left=80% top=0 width=30%}
...
[Applies to ALL lines in the ggplot<br>including `stat_smooth()`]{.note .absolute left=25% top=90%}

. . .

[Applies to only the `geom_point()` in the ggplot<br>not `stat_smooth()`]{.note .absolute left=75% top=90% width=30%}
...
[**Your Turn**<br>Combine any 3 figures]{.note .absolute left=75% top=23%}
...
- [R for Data Science](https://r4ds.had.co.nz) [by Hadley Wickham and Garret Grolemund]{.small}
